...
async def test_should_return401_on_not_auth_header(client: AsyncClient):
    response = await client.post(
        "/api/v1/esign/envelope/123/void",
        json={},
...
@pytest.mark.asyncio
async def test_should_return200_on_void_envelope(
    client: AsyncClient,
    ...
    with app_container.docusign_client.override(ds_client_mock):
        response = await client.post(
            f"/api/v1/esign/envelope/{pre_stored_envelopes['envelope_sent'].envelope_id}/void",
            json={"voidedReason": "Changed my mind"},
        )
...
async def test_should_return404_on_not_existing_envelope_in_db(
    ...
    envelope_id = "abcd4512-f63d-40e4-ab49-4c0eb5c99c5f"

    app_container.esign_envelope_service.reset()
    response = await client.post(
        f"/api/v1/esign/envelope/{envelope_id}/void",
        json={"voidedReason": "Changed my mind"},
    )
...
@pytest.mark.asyncio
async def test_should_return422_on_invalid_envelope_state(
    app_container: Container,
    mocker: MockerFixture,
    client: AsyncClient,
    mock_docusign_api_client,
    ...
    api_exc.body = json.dumps({"errorCode": ExcErrorCodeEnum.void_invalid_state.value})
    mocker.patch("docusign_esign.EnvelopesApi.update", side_effect=api_exc)
    ...
    assert response.json() == {
        "detail": {
            "message": "Only envelope in the 'Sent' or 'Delivered' states may be voided.",
            "field": "envelopeId",
            "value": pre_stored_envelopes["envelope_sent"].envelope_id,
        }
    }
...
async def test_should_return400_on_envelope_in_crating_state(
    ...
    app_container.esign_envelope_service.reset()
    response = await client.post(
        f"/api/v1/esign/envelope/{pre_stored_envelopes['envelope_creating'].envelope_id}/void",
        json={"voidedReason": "Changed my mind"},
    )
